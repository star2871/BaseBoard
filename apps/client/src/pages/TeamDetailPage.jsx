...
const TeamDetailPage = () => {
  ...
  if (loading) return <div className="flex justify-center items-center h-full"><Loader2 className="animate-spin mr-2" /> 팀 정보를 불러오는 중...</div>;
  if (error) return <div className="text-danger p-4 bg-red-900/20 rounded-md">에러: {error}</div>;
  if (!team) return <div className="text-warning">팀 데이터가 없습니다.</div>;
  ...
  return (
    <div>
      ...
      <div className="p-6 bg-surface-secondary rounded-lg border border-border-color mb-8">
        <p className="text-text-secondary"><strong>홈 구장:</strong> <span className="text-text-primary">{team.stadium || '정보 없음'}</span></p>
      </div>

      <div>
        <h2 className="text-2xl font-bold text-text-primary mb-4 flex items-center"><Users className="mr-2" /> 소속 선수 명단</h2>
        <div className="bg-surface-secondary rounded-lg border border-border-color">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-text-secondary uppercase"><tr><th scope="col" className="px-4 py-3">이름</th><th scope="col" className="px-4 py-3">포지션</th><th scope="col" className="px-4 py-3">상태</th><th scope="col" className="px-4 py-3">피로도</th></tr></thead>
            <tbody>
      ...
      <div className="mt-8">
        <h2 className="text-2xl font-bold text-text-primary mb-4 flex items-center"><BarChart2 className="mr-2" /> 최근 경기 기록</h2>
